...
struct IntPair {
    ...
    bool operator<(const IntPair& x)
        const {
        return this->i < x.i || (this->i
           == x.i && this->j < x.j);
    }
    bool operator==(const IntPair& x)
        const {
        ...
        //Ammeraal, Leen.Computer Graphics Programming in C++ / Qt(p. 78).UNKNOWN.Kindle Edition.
    }
};
struct Polygon3D {
    vector<int> nrs;
    // Vertex numbers (n elements) 
    qreal a, b, c, h;
    // of the plane equation
    // ax + by + cz = h
};
typedef set<IntPair,
    less<IntPair> >EdgeSet;
class Basic3D : public QMainWindow {
    Q_OBJECT
protected:
    Basic3D(QWidget* parent = 0);
    qreal xsMin, xsMax, ysMin, ysMax,
        xsRange = 0, ysRange,
        theta0Deg = 30,
        theta0 = theta0Deg * M_PI / 180,
        thetaDeg = theta0Deg,
        theta = theta0,  // Horizontal
        phi0Deg = 70,
        phi0 = phi0Deg * M_PI / 180,
        phiDeg = phi0Deg, phi = phi0,
        // Vertical
        xwMin, xwMax, ywMin, ywMax,
        zwMin, zwMax,
        xC, yC, zC, rho, rho0, rhoMin,
        rhoMax, d;
    const int hMenu = 21;
    int wDraw, hDraw,
        // Drawing in wDraw x hDraw pixels
        xDevCenter, yDevCenter;
    bool fileDialogDone = false;
    QString fileName = "";
    QMenu* viewMenu, * fileMenu;
    QAction* openAct, * exitAct,
        * incrDistAct, * decrDistAct,
    ...
    inline int xDev(qreal x)const {
        return xDevCenter +
            ...
            // Ammeraal, Leen.Computer Graphics Programming in C++ / Qt(pp. 78 - 79).UNKNOWN.Kindle Edition.
            int(d * (x - xsCenter));
    }
...
// Ammeraal, Leen.Computer Graphics Programming in C++ / Qt(p. 79).UNKNOWN.Kindle Edition.