...
void HLines::calculate() {  // called in thread
    int nPolyList = int(polyList.size());
    ...
    int nEdges = int(edges.size()), iEdges = 0;
...
void HLines::exportSVG() {
    ...
    *pOutSVG << "<svg width=\"" << width() << "\" height=\""
        ...
        << height() << "\"" << Qt::endl
        << "xmlns=\"http://www.w3.org/2000/svg\"" << Qt::endl
...
void HLines::collectTrias(int i, int j) {
    ...
    int nrel = int(relatedtrianrs.size()), nrect = int(rectTrianrs.size());
