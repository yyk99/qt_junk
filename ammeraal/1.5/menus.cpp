...
Menus::Menus(QWidget* parent) :
    ...
    setGeometry(w / 10, h / 10,
       8 * w / 10, 8 * h / 10);
    setWindowTitle(
       "A simple file menu in action");
    // Create actions:
    QAction* openAct = new QAction(
       "Open...", this);
    openAct->setShortcut(
       Qt::CTRL | Qt::Key_O);
    connect(openAct, &QAction::
       triggered, this, &Menus::open);

    QAction* saveAct =
        new QAction("Save...", this);
    saveAct->setShortcut(
       Qt::CTRL | Qt::Key_S);
    connect(saveAct, &QAction::
       triggered, this, &Menus::save);
    QAction* exitAct =
    ...
    connect(
       exitAct, &QAction::triggered,
          this, &QWidget::close);
    QAction* infoAct = new QAction(
         "Info", this);
    connect(
       infoAct, &QAction::triggered,
          this, &Menus::info);
    // Create menus:
    QMenu* fileMenu =
...
void Menus::info()const {
    ...
    msgBox.setText("Demo of menus and"
              " file input/output\n");
    msgBox.exec();
}

// Ammeraal, Leen.Computer Graphics Programming in C++ / Qt(p. 21).UNKNOWN.Kindle Edition.