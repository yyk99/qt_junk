...
class DrawPolygon : public QMainWindow {
    Q_OBJECT
public:
    DrawPolygon(QWidget* parent = 0);
protected:
    void showCoords(QWidget* p)const;
    int W, H;
    qreal w, h, c;
    bool outsideLogicalBoundaries(
    ...
    void keyPressEvent(
       QKeyEvent*) override;
    bool samePoint(const Point2D& P,
       const Point2D& Q)const;
    QString str(qreal x)const;
};
#endif // DRAWPOLYGON_H

// Ammeraal, Leen.Computer Graphics Programming in C++ / Qt(p. 38).UNKNOWN.Kindle Edition.